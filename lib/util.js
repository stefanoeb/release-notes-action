...
function getSpecificVersionDraft(lastReleases, versionBeingDrafted) {
  if (lastReleases && lastReleases.length > 0) {
    return lastReleases.find(release => release.name === versionBeingDrafted);
  }
}

function generateNewReleaseDescription(message, lastReleaseDescription) {
...
function validateCommitMessage(commitMessage, latestReleaseDescription) {
  ...
  if (latestReleaseDescription && latestReleaseDescription.includes(commitMessage)) {
