...
async function getLatestReleases() {
  try {
    ...
    if (releases.length === 0) {
      console.log('📭 FYI: No releases found so far in your repo');
    }
    return releases;
  } catch (e) {
...
async function generateDraft({ version, repoName, gt }) {
  console.log(`release-manager: 🗒️ Create draft ${version} from branch: develop`);
  const response = execSync(
    `curl -sH 'Authorization: token ${gt}' -d '{
        "tag_name": "",
...
async function updateReleaseDescription({
  gt, version, releaseNotes, repoName, draftId,
}) {
  const response = execSync(
    `curl -sH 'Authorization: token ${gt}' -d '{
        "tag_name": "",
